...
public class King extends Piece {
  ...
  public King(){
    super("King","White",'E', 1);
  }
  public King(String type, String color, char x_move, int y_move){
  ...
  public boolean checkMovement(int x, int y){
    ...
    if(Math.abs(x-temp)<= 1 && Math.abs(y-getpos_y())<= 1 || Math.abs(temp-x) == Math.abs(getpos_y()-y) && Math.abs(x-temp)<= 1 && Math.abs(y-getpos_y())<= 1){
